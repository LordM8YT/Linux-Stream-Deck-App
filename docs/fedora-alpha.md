...
```bash
sudo dnf install ./dist/OpenDeck-0.1.0-alpha.4-linux-x86_64.rpm
```
...
```bash
chmod +x ./dist/OpenDeck-0.1.0-alpha.4-linux-x86_64.AppImage
./dist/OpenDeck-0.1.0-alpha.4-linux-x86_64.AppImage
```
...
The workflow `.github/workflows/alpha-release.yml` builds the Fedora alpha artifacts on GitHub Actions, uploads them as downloadable build artifacts, and can publish them to a GitHub pre-release when you push a version tag.
